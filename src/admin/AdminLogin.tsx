...
export default function AdminLogin({ onLogin }: P) {
  ...
  const go = (ev: React.FormEvent) => {
    ev.preventDefault();
    // Passwordless admin access for hidden backend entry; use a real auth flow for production.
    ...
    <div className="flex min-h-screen items-center justify-center bg-navy-950 px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={go} className="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur">
          ...
          <div className="mb-4">
            <label className="block text-sm text-navy-300 mb-1.5">Username</label>
            <input
              value={u}
          ...
          <p className="mt-3 text-center text-xs text-navy-400">
            If the page still shows an error, try clearing browser cache or opening in a private window.
          </p>
        </form>
      </div>
    </div>
