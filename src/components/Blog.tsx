          ...
          {data.blogs.slice(0,3).map((b,i)=>(
            <motion.article key={b.id} initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.1}}
              ...
              <div className="p-6">
                ...
                <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-yellow-600">Read more <ArrowRight className="h-3 w-3 group-hover:translate-x-0.5 transition-transform"/></span>
              </div>
            </motion.article>
          ))}
        </div>
